...
@jellyfin_bp.route('/test-active', methods=['POST'])
def test_active_connection():
    ok, server_name = current_app.jellyfin_service.test_active_connection()
    if ok:
        return jsonify(connected=True, serverName=server_name)
    return jsonify(connected=False, error='Could not reach Jellyfin server')


@jellyfin_bp.route('/refresh', methods=['POST'])
def refresh():
    ok, error, libraries = current_app.jellyfin_service.refresh_connection()
    if ok:
