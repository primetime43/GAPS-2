...
import { SonarrService, SonarrTag } from '../../../services/sonarr.service';
import { SonarrSettingsComponent } from './sonarr-settings.component';

describe('Sonarr defaults and library mappings', () => {
  let fixture: ComponentFixture<SonarrSettingsComponent>;
  let component: SonarrSettingsComponent;
  let service: jasmine.SpyObj<SonarrService>;

  beforeEach(async () => {
    service = jasmine.createSpyObj('SonarrService', [
    ...
    service.getRootFolders.and.returnValue(of([{ path: '/tv', free_space: 0, accessible: true }]));
    service.getTags.and.returnValue(of([{ id: 7, label: 'gaps' }, { id: 2, label: 'requests' }]));
    service.getLibraries.and.returnValue(of([{ source: 'plex', server: 'Plex', library: 'TV Shows' }]));
    await TestBed.configureTestingModule({
      imports: [CommonModule, FormsModule],
      declarations: [SonarrSettingsComponent],
      providers: [{ provide: SonarrService, useValue: service }],
    }).compileComponents();
    fixture = TestBed.createComponent(SonarrSettingsComponent);
    component = fixture.componentInstance;
    service.getConfig.and.returnValue(of({
      ...component.config, enabled: true, url: 'http://sonarr.test', api_key: 'key', tags: [7],
    }));
  ...
  it('loads selected tags and saves multiple selections as numeric IDs', () => {
    const select: HTMLSelectElement = fixture.nativeElement.querySelector('#sonarrTags');
  ...
  it('keeps saved tags and other defaults usable when tags cannot load', async () => {
    service.getTags.and.returnValue(throwError(() => ({ error: { error: 'Sonarr offline' } })));
    component.loadMeta();
    fixture.detectChanges();
    await fixture.whenStable();
    expect(component.config.tags).toEqual([7]);
    expect(component.tagsError).toBe('Sonarr offline');
    expect(component.profiles[0].id).toBe(1);
    expect(component.rootFolders[0].path).toBe('/tv');
    expect(fixture.nativeElement.querySelector('#sonarrTags').disabled).toBeTrue();
    component.saveConfig();
  ...
  it('waits for tags and refreshes options without resetting saved IDs', () => {
    const tags = new Subject<SonarrTag[]>();
  ...
  it('saves a library mapping chosen in the form and allows it to be removed', async () => {
    const select: HTMLSelectElement = fixture.nativeElement.querySelector('#sonarrLibraryRoot0');
    select.value = '/tv';
    select.dispatchEvent(new Event('change'));
    component.saveConfig();
    expect(service.saveConfig.calls.mostRecent().args[0].library_root_folders).toEqual([
      { source: 'plex', server: 'Plex', library: 'TV Shows', root_folder_path: '/tv' },
    ]);
    select.value = '';
    select.dispatchEvent(new Event('change'));
    component.saveConfig();
  ...
  it('preserves mappings when library discovery fails, including disconnected servers', () => {
    component.config.library_root_folders = [
      { source: 'plex', server: 'Old Plex', library: 'TV Shows', root_folder_path: '/old' },
    ];
  ...
  it('keeps identically named libraries on different servers separate', () => {
    const first = { source: 'plex', server: 'Plex', library: 'TV Shows' };
    const second = { ...first, server: 'Another Plex' };
    component.setMappedRoot(first, '/tv');
    component.setMappedRoot(second, '/other');
    expect(component.mappedRoot(first)).toBe('/tv');
